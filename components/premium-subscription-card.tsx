...
import { 
  ...
  ArrowRight, RefreshCw, Gift, Flame
} from "lucide-react"
...
export function PremiumSubscriptionCard({ className }: PremiumSubscriptionCardProps) {
  ...
  const getSubscriptionStatusBadge = () => {
    ...
    if (status === 'active') {
      if (tier === 'premium') {
        return subscription.billing_cycle === 'lifetime' 
          ? <Badge className="bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 border-0">
              <Sparkles className="h-3 w-3 mr-1" />
              Premium Lifetime
            </Badge>
          : <Badge className="bg-slate-700 dark:bg-slate-300 text-white dark:text-slate-900 border-0">
  ...
  return (
    <div className={cn("space-y-8", className)}>
      {/* Upgrade to Lifetime Card - Only show to premium users who don't have lifetime */}
      {isPremium && subscription?.billing_cycle !== 'lifetime' && (
      ...
      <Card className="shadow-lg border border-slate-200 dark:border-slate-700">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
            {subscription && subscription.billing_cycle !== 'lifetime' ? (
              <>
                <Button
