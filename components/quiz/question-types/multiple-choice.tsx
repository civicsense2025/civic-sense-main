...
export function MultipleChoiceQuestion({
  ...
  return (
    <RadioGroup value={selectedAnswer || ""} onValueChange={onSelectAnswer} className="space-y-3" role="radiogroup" aria-labelledby={`question-title`}>
      {randomizedOptions.map((option, index) => {
        const isCorrect = isSubmitted && option.label === question.correct_answer
        const isIncorrect = isSubmitted && selectedAnswer === option.label && option.label !== question.correct_answer
