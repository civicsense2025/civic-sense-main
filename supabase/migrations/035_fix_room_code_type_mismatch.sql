-- =============================================================================
-- FIX ROOM CODE TYPE MISMATCH
-- =============================================================================
-- Fixes the type mismatch where room_code is VARCHAR(8) in the table but 
...
-- Recreate with correct VARCHAR(8) type to match the table column
...
RETURNS TABLE(
  id UUID,
  room_code VARCHAR(8),  -- Changed from TEXT to VARCHAR(8) to match table column
  topic_id TEXT,
  room_name TEXT,
  max_players INTEGER,
...
DECLARE
  v_room_code VARCHAR(8);  -- Changed from TEXT to VARCHAR(8)
  v_room_id UUID;
BEGIN
...
-- Also fix the generate_room_code function to return VARCHAR(8)
DROP FUNCTION IF EXISTS generate_room_code CASCADE;

CREATE OR REPLACE FUNCTION generate_room_code()
RETURNS VARCHAR(8) AS $$  -- Changed from TEXT to VARCHAR(8)
DECLARE
  code VARCHAR(8);  -- Changed from TEXT to VARCHAR(8)
  attempts INTEGER := 0;
...
-- Also fix the join_multiplayer_room function parameter type
DROP FUNCTION IF EXISTS join_multiplayer_room CASCADE;

CREATE OR REPLACE FUNCTION join_multiplayer_room(
  p_room_code VARCHAR(8),  -- Changed from TEXT to VARCHAR(8)
  p_player_name TEXT,
...
COMMENT ON FUNCTION create_multiplayer_room IS 'Creates multiplayer rooms with VARCHAR(8) room_code type to match table column';
COMMENT ON FUNCTION generate_room_code IS 'Generates unique 8-character room codes as VARCHAR(8)';
COMMENT ON FUNCTION join_multiplayer_room IS 'Joins players to multiplayer rooms using VARCHAR(8) room_code'; 