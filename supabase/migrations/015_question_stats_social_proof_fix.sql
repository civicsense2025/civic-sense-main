...
-- Fixed version that handles sample data insertion more gracefully
...
SELECT 
  ...
  -- Most common wrong answer (using a simpler approach)
  (
    SELECT uqr2.user_answer 
    FROM user_question_responses uqr2 
    WHERE uqr2.question_id = q.id AND uqr2.is_correct = false 
    GROUP BY uqr2.user_answer 
    ORDER BY COUNT(*) DESC 
    LIMIT 1
  ) as most_common_wrong_answer,
...
-- Optional: Create sample data only if questions exist and user_question_responses table is ready
-- This is wrapped in a safer block that won't fail the migration
DO $$
DECLARE
...
BEGIN
  ...
  SELECT COUNT(*) INTO question_count FROM questions LIMIT 1;
  
  -- Only proceed if we have questions
  IF question_count > 0 THEN
    -- Create a sample user for responses (or use existing one)
    SELECT id INTO sample_user_id FROM profiles LIMIT 1;
    
    -- If no profiles exist, create a sample one
    IF sample_user_id IS NULL THEN
      INSERT INTO profiles (id, full_name) 
      VALUES (gen_random_uuid(), 'Sample User') 
      RETURNING id INTO sample_user_id;
    END IF;
    
    -- Now create sample quiz attempt and responses
    DECLARE
    ...
    BEGIN
      ...
      INSERT INTO user_quiz_attempts (
        id, user_id, topic_id, total_questions, started_at, is_completed
      ) VALUES (
        gen_random_uuid(), sample_user_id, 'sample-topic', 10, NOW(), true
      ...
      LOOP
        ...
        FOR i IN 1..LEAST(total_responses, 100) LOOP -- Cap at 100 to avoid timeout
          BEGIN
            INSERT INTO user_question_responses (
              ...
              10 + floor(random() * 110),
              -- Use our sample attempt ID
              attempt_id
            );
          EXCEPTION WHEN OTHERS THEN
            -- If insert fails (e.g., constraint violation), just continue
            NULL;
          END;
        END LOOP;
        
        RAISE NOTICE 'Created sample responses for question %', question_record.id;
      END LOOP;
    END;
  ELSE
    RAISE NOTICE 'No questions found - skipping sample data creation';
  END IF;
  
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Sample data creation failed (this is ok): %', SQLERRM;
END;
$$;
