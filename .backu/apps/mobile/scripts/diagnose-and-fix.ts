 ...
 * 
 * Usage:
 *   npx tsx scripts/diagnose-and-fix.ts
 *   npx tsx scripts/diagnose-and-fix.ts --fix
 *   npx tsx scripts/diagnose-and-fix.ts --fix-categories
 *   npx tsx scripts/diagnose-and-fix.ts --fix-sources
 */
...
async function diagnoseNavigationIssues(): Promise<DiagnosticResult> {
  ...
  console.log(`  🧭 ${incorrectlyClassified.length}/${problematicIds.length} IDs incorrectly classified`);
...
function displayResults(results: DiagnosticResult[]): void {
  ...
  if (autoFixableIssues > 0) {
    ...
    console.log('   npx tsx scripts/diagnose-and-fix.ts --fix');
    console.log('   npx tsx scripts/diagnose-and-fix.ts --fix-categories');
  }
}
...
async function main(): Promise<void> {
  try {
    ...
    if (shouldFix || fixCategories) {
      ...
      console.log('   npx tsx scripts/diagnose-and-fix.ts');
    }
