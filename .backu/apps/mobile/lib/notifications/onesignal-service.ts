...
class MobileOneSignalService {
  ...
  async initialize(appId: string): Promise<void> {
    try {
      ...
      if (typeof window !== 'undefined' && window.location) {
        console.log('🌐 Web platform detected - skipping OneSignal mobile service initialization');
  ...
  private setupUserStateHandlers(): void {
    try {
      // Handle push subscription changes with try-catch for API compatibility
      try {
        OneSignal.User.addEventListener('change', (event: any) => {
          console.log('📱 User state changed:', event);
          
          // Handle push token changes
          if (event.current?.pushSubscription?.token) {
            this.updateUserSubscriptionStatus(true, event.current.pushSubscription.token);
          }
          
          // Handle OneSignal ID changes
          if (event.current?.onesignalId) {
            this.updateUserOneSignalId(event.current.onesignalId);
          }
        });
      } catch (error) {
        console.warn('OneSignal user state change listener not supported:', error);
      }
    } catch (error) {
      console.warn('OneSignal user state handlers setup failed:', error);
    }
  }
