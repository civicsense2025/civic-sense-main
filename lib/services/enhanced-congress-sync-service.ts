...
export class EnhancedCongressSyncService {
  ...
  async performComprehensiveSync(options: {
    ...
    console.log(`🚀 Starting comprehensive congressional sync for congresses: ${congressNumbers.join(', ')}`);
    ...
    try {
      ...
      for (const congressNumber of congressNumbers) {
        console.log(`\n📊 Processing ${congressNumber}th Congress...`);
        
        // 1. Sync congressional members (foundation for everything else)
        if (syncMembers) {
          console.log(`👥 Syncing congressional members for ${congressNumber}th Congress...`);
        ...
        if (syncBills) {
          console.log(`📜 Syncing bills with GovInfo enhancement for ${congressNumber}th Congress...`);
        ...
        if (syncCommitteeDocuments) {
          console.log(`📋 Syncing committee documents for ${congressNumber}th Congress...`);
      ...
      if (generateEvents) {
        console.log('📅 Generating events from documents...');
      ...
      if (generateQuizContent) {
        console.log('📝 Generating quiz content from documents...');
  ...
  private async generateQuizContentForBill(billData: any, govInfoPackage: any): Promise<void> {
    try {
      ...
      console.log(`✅ Quiz content generated for bill: ${billData.title}`);
    } catch (error) {
  ...
  private async generateQuizContentForHearing(hearing: any, hearingDoc: any): Promise<void> {
    try {
      ...
      console.log(`✅ Quiz content generated for hearing: ${hearing.hearing_title}`);
    } catch (error) {
  ...
  private async generateQuizContentForDocument(documentRecord: any, document: any): Promise<void> {
    try {
      ...
      console.log(`✅ Quiz content generated for document: ${documentRecord.document_title}`);
    } catch (error) {
