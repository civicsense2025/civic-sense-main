...
export class CongressAPIClient {
  ...
  /**
   * Format date for Congress API (removes milliseconds)
   * Congress API expects: YYYY-MM-DDTHH:MM:SSZ
  ...
  async getBills(params: {
    ...
    Object.entries(params).forEach(([key, value]) => {
      if (value !== undefined && key !== 'congress') {
        ...
        if ((key === 'fromDateTime' || key === 'toDateTime') && typeof value === 'string') {
          queryParams.append(key, this.formatDateForAPI(value));
        } else {
