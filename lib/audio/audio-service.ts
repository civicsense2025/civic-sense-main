...
// Optional TrackPlayer import - gracefully handle if package is not available in Expo Go or web
...
if (!isWeb) {
  try {
    ...
    Track = TrackPlayerModule.Track;
    AppKilledPlaybackBehavior = TrackPlayerModule.AppKilledPlaybackBehavior;
  } catch (error) {
    console.warn('⚠️ react-native-track-player not available (requires development build). Background music disabled.');
  }
}
...
class MobileAudioService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // Configure audio session for iOS only
      if (Platform.OS === 'ios') {
      ...
      // Initialize TrackPlayer for music (only on native platforms)
      if (isNative) {
        await this.initializeTrackPlayer();
      } else {
        console.log('🌐 Running on web - skipping TrackPlayer initialization');
      }
      
      // Load sound effects (only on native platforms)
      if (isNative) {
        await this.loadSoundEffects();
      } else {
        console.log('🌐 Running on web - skipping sound effects loading');
      }
      ...
      console.warn('⚠️ Audio service initialization failed, continuing with limited functionality:', error);
      // Don't throw on web platform - allow the app to continue
      if (isWeb) {
        this.isInitialized = true; // Mark as initialized with limited functionality
        console.log('🌐 Audio service running in web compatibility mode');
      } else {
        throw error;
      }
    }
  ...
  private async initializeTrackPlayer(): Promise<void> {
    try {
      // Skip TrackPlayer initialization if not available
      if (!TrackPlayer) {
        console.warn('⚠️ TrackPlayer not available, skipping background music setup');
        return;
      }

      // Check if setupPlayer method exists before calling
      if (typeof TrackPlayer.setupPlayer !== 'function') {
        console.warn('⚠️ TrackPlayer.setupPlayer not available - incomplete TrackPlayer module');
        return;
      }
      ...
      if (!AppKilledPlaybackBehavior || !Capability || !Event) {
        console.warn('⚠️ TrackPlayer constants not available - skipping advanced configuration');
        return;
      }
  ...
  private async loadSoundEffects(): Promise<void> {
    try {
      for (const soundDef of this.civicSounds) {
        try {
          // Skip sounds with null file references
          if (!soundDef.file) {
            console.log(`⏭️ Skipping sound ${soundDef.name} - no file available yet`);
            continue;
          }
  ...
  async speakText(text: string, options?: { 
    ...
    try {
      if (!this.isInitialized) {
        console.warn('⚠️ Audio service not initialized, skipping TTS');
        return;
      }
