...
import { WaitingRoom } from "@civicsense/ui-web/components/multiplayer/waiting-room"
import { QuizEngine } from "@civicsense/ui-web/components/quiz/quiz-engine"
import { useMultiplayerRoom } from "@civicsense/shared/lib/multiplayer"
import { dataService } from "@civicsense/shared/lib/data-service"
import type { QuizQuestion, TopicMetadata } from "@civicsense/shared/lib/quiz-data"
...
function MultiplayerQuizClient({ params, searchParams }: MultiplayerQuizClientProps) {
  ...
  const handleSelfJoin = async () => {
    ...
    setIsJoining(true)
    try {
      const result = await (await import('@civicsense/shared/lib/multiplayer')).multiplayerOperations.joinRoom({
        roomCode: roomId,
  ...
  if (questions.length > 0 && topic) {
    ...
    const MultiplayerQuizRouter = React.lazy(() => 
      import('@civicsense/ui-web/components/multiplayer/multiplayer-quiz-router').then(module => ({
