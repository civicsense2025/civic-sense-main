...
// Optional OneSignal import - gracefully handle if package is not installed
let OneSignal: any = null;
let OSNotification: any = null;

try {
  const OneSignalModule = require('react-native-onesignal');
  OneSignal = OneSignalModule.OneSignal;
  OSNotification = OneSignalModule.OSNotification;
} catch (error) {
  console.warn('⚠️ OneSignal package not installed. Push notifications will be disabled.');
}
...
export function OneSignalProvider({ children }: OneSignalProviderProps) {
  ...
  try {
    ...
    console.warn('⚠️ OneSignalProvider: useAuth not available yet, continuing without user context');
  }
  ...
  const initializeOneSignal = async () => {
    try {
      ...
      if (typeof window !== 'undefined' && window.location) {
        console.log('🌐 Web platform detected - skipping OneSignal initialization');
      ...
      if (!OneSignal) {
        console.warn('OneSignal not available - skipping push notification setup');
        setIsInitialized(true);
        return;
      }

      // Initialize OneSignal only on mobile platforms
      console.log('📱 Mobile platform detected - initializing OneSignal');
  ...
  const requestPermission = async () => {
    try {
      if (!OneSignal) {
        console.warn('OneSignal not available - cannot request permission');
        return;
      }
  ...
  const updateUserSubscription = async () => {
    ...
    try {
      // Set external user ID in OneSignal (if available)
      if (OneSignal) {
  ...
  const syncCivicTags = async () => {
    ...
    try {
      ...
      if (subscription) {
        ...
        // Set tags in OneSignal (if available)
        if (OneSignal) {
  ...
  const trackCivicEvent = async (eventType: string, data?: any) => {
    ...
    try {
      ...
      // Also track as a custom event in OneSignal (if available)
      if (OneSignal) {
