...
`.trim();
    ...
    console.log(`📅 Created calendar event for topic ${topic.topic_id}: ${eventResult.htmlLink}`);
  }
  ...
  private async findCivicSenseEvents(): Promise<Array<{ id: string; summary: string }>> {
    ...
    // Search for events with CivicSense in the title
    const response = await fetch(
    ...
    return (data.items || [])
      .filter((event: any) => 
        ...
        event.extendedProperties?.private?.civicsense_app
      )
      .map((event: any) => ({
        id: event.id,
