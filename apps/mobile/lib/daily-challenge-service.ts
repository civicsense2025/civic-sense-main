...
export class DailyChallengeTracker {
  ...
  async getIncompleteQuizzesForToday(userId: string): Promise<{ data: TopicWithProgress[]; error: any }> {
    try {
      ...
      // Map to TopicWithProgress format
      const incompleteTopics: TopicWithProgress[] = incompleteProgress
  ...
  /**
   * Get leaderboard data for daily challenges
   */
...
export const dailyChallengeService = DailyChallengeTracker.getInstance(); 